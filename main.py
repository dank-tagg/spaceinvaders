...
def main():
    ...
    def redraw_window():
        ...
        for n in range(lives):
            window.blit(pygame.transform.scale(pygame.image.load('./assets/heart.png'), (125, 50)), (10+n*25, 0))
        window.blit(level_label, (width - level_label.get_width() - 10, 10))
        window.blit(score_label, (width - level_label.get_width() - 250, 10))
...
def main_menu():
    ...
    while run:
        ...
        subtitle_label = title_font.render('Use WASD keys to move', 1, (255, 255, 255))
        window.blit(title_label, (width/2 - title_label.get_width()/2, 350))
        window.blit(subtitle_label, (width/2 - subtitle_label.get_width()/2, 500))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
